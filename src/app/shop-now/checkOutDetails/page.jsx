...
const Shop = () => {
  ...
  const createPaymentOrder = async (amount, name) => {
    try {
      ...
      const response = await fetch("https://backend-tendoni-backend.ffbufe.easypanel.host/web/api/v1/CreatePaymentData", {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
  ...
  const verifyPayment = async (payment_id, order_id, signature) => {
    ...
    try {
      ...
      console.log(signature,"payment signature,orderID")
      const response = await fetch("https://backend-tendoni-backend.ffbufe.easypanel.host/web/api/v1/verifyPayment", {
        method: 'POST',
        headers: {
    ...
    try {
      ...
      const options = {
        ...
        handler: async (response) => {
          ...
          try {
            const verificationResponse = await verifyPayment( razorpay_payment_id, razorpay_order_id,razorpay_signature);
  ...
  const removeItem = async (cartId, itemId) => {
    try {
      ...
      const { access_token } = userData.data;
      const response = await fetch('https://backend-tendoni-backend.ffbufe.easypanel.host/web/api/v1/removeSingleCartItem', {
        method: "DELETE",
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
                ...
                <div className="address-box">
                  <div className="flex flex-wrap">
                    {customerData.map((item, index) => (
                      <div className="address-list col-lg-6 md:col-lg-2 sm:col-lg-2" key={index}>
                ...
                {cartData && cartData.items?.length > 0 ? (
                  cartData.items.map((item, index) => (
                    <div key={index} className="flex items-center justify-between">
                      ...
                      <div>
                        <p className="price">
                          <span style={{ textDecorationLine: 'line-through' }}>Rs. {item.maxPrice}</span>
                        </p>
                      </div>
                      <div>
