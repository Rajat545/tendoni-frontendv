...
export function CartProvider({ children }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://api.tendonigroup.com/web/api/v1/getAllSpicesProduct');
        if (!response.ok) {
  ...
  const signUp = async (e) => {
    ...
    try {
      const response = await fetch('https://api.tendonigroup.com/web/api/v1/signup', {
        method: 'POST',
